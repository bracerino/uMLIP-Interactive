    ...
    if st.session_state.results:
        ...
        if phonon_results:
            ...
            if phonon_data.get('thermal_properties_dict'):
                ...
                with col_temp1:
                    min_temp = st.number_input("Min Temperature (K)", min_value=0, max_value=2000, value=0, step=10, key=f"min_temp_{selected_phonon['name']}")
                with col_temp2:
                    max_temp = st.number_input("Max Temperature (K)", min_value=100, max_value=2000, value=1000, step=50, key=f"max_temp_{selected_phonon['name']}")
                with col_temp3:
                    temp_step = st.number_input("Temperature Step (K)", min_value=1, max_value=100, value=10, step=1, key=f"temp_step_{selected_phonon['name']}")
