...
def create_cell_filter(atoms, optimization_params):
    """Create appropriate cell filter based on optimization parameters"""
    ...
    pressure_eV_A3 = pressure * 0.00624150913  # GPa to eV/Å³

    if cell_constraint == "Full cell (lattice + angles)":
        if hydrostatic:
            # Hydrostatic strain - only uniform scaling
...
def setup_optimization_constraints(atoms, optimization_params):
    ...
    if opt_type == "Atoms only (fixed cell)":
        ...
        # No atomic constraints, use cell filter
        cell_filter = create_cell_filter(atoms, optimization_params)
        return cell_filter, "both"


class CellOptimizationLogger:
    """Extended logger for cell optimization that tracks both forces and stress"""
    ...
    def __call__(self, optimizer=None):
        if optimizer is not None:
            # Get the underlying atoms object
            if hasattr(optimizer.atoms, 'atoms'):
                atoms = optimizer.atoms.atoms  # For filtered objects
            else:
                atoms = optimizer.atoms
            ...
            try:
                ...
                # Get stress for cell optimization
                stress = None
                max_stress = 0.0
                if self.opt_mode in ["cell_only", "both"]:
                    try:
                        stress_voigt = atoms.get_stress(voigt=True)  # Get stress in Voigt notation
                        stress = stress_voigt
                ...
                self.log_queue.put({
                    ...
                    'energy': energy,
                    'max_force': max_force,
                    'max_stress': max_stress,  # Make sure this is included
                    'energy_change': energy_change,
                    'total_steps': None
                })
            ...
            try:
                ...
                if calc_type == "Energy Only":
                    ...
                    try:
                        ...
                        # Setup optimization object and constraints
                        ...
                        if opt_type == "Cell only (fixed atoms)":
                            ...
                            # For cell-only optimization, use stress-based convergence

                            fmax_criterion = 0.1  # Relaxed force criterion since atoms are fixed
                        ...
                        if hasattr(optimization_object, 'atoms'):
                            ...
                            final_atoms = optimization_object.atoms  # For filtered objects
                        ...
                        # Check stress convergence for cell optimization
                        ...
                        optimized_structure = Structure(
                            ...
                            lattice=final_atoms.cell[:],

                            species=[atom.symbol for atom in final_atoms],

                            coords=final_atoms.positions,
...
with tab1:
    ...
    if st.session_state.structures:
        ...
        if calc_type == "Geometry Optimization":
            ...
            # Add cell optimization parameters
            ...
            if optimization_type in ["Cell only (fixed atoms)", "Both atoms and cell"]:
                ...
                # Pressure option for cell optimization
                col_press1, col_press2, col_press3 = st.columns(3)  # Change to 3 columns
                with col_press1:
        ...
        if isinstance(message, dict):
            if message.get('type') == 'progress':
                ...
                st.session_state.current_optimization_info = {
                    ...
                    'optimization_type': message.get('optimization_type', 'Both atoms and cell'),  # Add this
                    'stress_threshold': message.get('stress_threshold', 0.1),  # Add this
                    'is_optimizing': True
                }
                ...
                st.session_state.optimization_steps[structure_name].append({
                    ...
                    'max_stress': message.get('max_stress', 0.0),  # Add stress tracking
                ...
                if st.session_state.current_optimization_info.get('structure') == structure_name:
                    st.session_state.current_optimization_info.update({
                        ...
                        'current_max_stress': message.get('max_stress', 0.0),  # Add stress to current info
    ...
    if st.session_state.calculation_running:
        ...
        if st.session_state.current_optimization_info and st.session_state.current_optimization_info.get(
            ...
            if 'current_step' in opt_info and opt_info['current_step'] > 0:
                # Determine number of columns based on optimization type
...
with tab3:
    ...
    if st.session_state.results:
        # Create nested tabs within the Results tab
        results_tab1, results_tab2, results_tab3, results_tab4 = st.tabs(["📊 Energies", "🔧 Geometry Optimization Details", "Elastic properties", "Phonons"])
    ...
    if st.session_state.results:
        ...
        with results_tab2:
            with results_tab2:
                ...
                # Filter for geometry optimization results
                ...
                if geometry_results:
                    ...
                    # Lattice parameters comparison table
                    st.subheader("📐 Lattice Parameters Comparison")

                    lattice_data = []
                    for result in geometry_results:
                        try:
                            # Get initial structure (from st.session_state.structures)
                    ...
                    if lattice_data:
                        ...
                        # Download lattice comparison as CSV
                    ...
                    # Lattice parameter changes visualization
                    if len(lattice_data) > 1:
                        ...
                        with col_vis1:
                            # Plot lattice parameter changes
                        ...
                        with col_vis2:
                            ...
                            fig_volume.add_trace(go.Bar(
                                ...
                                textposition='auto',
                                textfont=dict(size=16)  # Added text font size
                            ))
                    ...
                    # Download optimized structures section
                    st.subheader("📁 Download Optimized Structures")
