...
def extract_orbmol_results(atoms, calculator, log_queue, structure_name):
    """Extract OrbMol-v2 per-atom latent charges (captured via forward hook)."""
    try:
        res = atoms.calc.results if (hasattr(atoms, 'calc') and atoms.calc is not None) \
            else calculator.results
        charges = res.get('latent_charges')
        if charges is None:
            return {'success': False, 'error': 'latent_charges not captured (model has no LatentChargeHead?)'}
        charges = np.array(charges).reshape(-1)
        log_queue.put(
            f"  OrbMol latent charges: min={charges.min():.4f} e, "
            f"max={charges.max():.4f} e, sum={charges.sum():.4f} e"
        )
        return {
            'success': True,
        ...
        log_queue.put(f"  ⚠️ OrbMol charge extraction failed: {e}")
        return {'success': False, 'error': str(e)}
...
# --- per-displacement force cache ------------------------------------------
# One SCF per displaced supercell is what makes a DFT phonon run expensive, so
# every finished displacement is written to its own folder. An interrupted run
# (Ctrl-C, a full disk, a lost node) restarted with the same settings then skips
# straight to the first displacement that has no forces yet.

# Positions/cell agreement required before cached forces are accepted. Loose
# enough to survive the text round-trip of the cached pre-relaxed structure,
# far tighter than any displacement (0.01 Å) it has to tell apart.
PHONON_CACHE_TOL = 1e-6


def phonon_force_cache_dir(calculator, phonon_params, structure_name, atoms, log_queue):
    """Cache folder for this run, or None when caching does not apply.

    Only Quantum ESPRESSO gets one: a force field recomputes a displacement in
    milliseconds, so there is nothing to save and no folders worth leaving behind.
    """
    if not any(c.__name__ == 'Espresso' for c in type(calculator).__mro__):
        return None
    if not phonon_params.get('resume_from_cache', True):
        return None
    try:
        cache_dir = qe.qe_phonon_cache_dir(structure_name, phonon_params, atoms=atoms)
        os.makedirs(cache_dir, exist_ok=True)
        return cache_dir
    except Exception as exc:
        log_queue.put(f"  ⚠️ Could not open the force cache ({exc}) — running without resume")
        return None
...
def _phonon_cache_matches(cached_positions, cached_cell, cached_numbers, ase_sc):
    return (
        np.array_equal(cached_numbers, ase_sc.get_atomic_numbers())
        and np.allclose(cached_cell, np.array(ase_sc.get_cell()), atol=PHONON_CACHE_TOL)
        and np.allclose(cached_positions, ase_sc.get_positions(), atol=PHONON_CACHE_TOL)
    )


def phonon_cache_load_forces(slot, ase_sc):
    """Forces stored for exactly this displaced supercell, or None."""
    path = os.path.join(slot, 'forces.npz')
    if not os.path.isfile(path):
        return None
    try:
        with np.load(path) as data:
            forces = data['forces']
            if forces.shape != (len(ase_sc), 3):
                return None
            if not _phonon_cache_matches(data['positions'], data['cell'],
                                         data['numbers'], ase_sc):
                return None
            return forces
    except Exception:
...
def phonon_cache_store_forces(slot, ase_sc, forces, calculator, log_queue):
    """Save the forces plus the pw.x input/output of this displacement."""
    try:
        os.makedirs(slot, exist_ok=True)

        source_dir = str(getattr(calculator, 'directory', '') or '')
        for fname in ('espresso.pwi', 'espresso.pwo'):
            source = os.path.join(source_dir, fname)
            if os.path.isfile(source):
                shutil.copy2(source, os.path.join(slot, fname))

        # Written aside and renamed, so an interrupted write can never leave a
        # truncated forces.npz that a later run would have to distrust.
        tmp = os.path.join(slot, '_forces.partial.npz')
        np.savez(
            tmp,
            forces=np.asarray(forces, dtype=float),
            positions=ase_sc.get_positions(),
            cell=np.array(ase_sc.get_cell()),
            numbers=ase_sc.get_atomic_numbers(),
        )
        os.replace(tmp, os.path.join(slot, 'forces.npz'))
    except Exception as exc:
        log_queue.put(f"    ⚠️ Could not cache displacement forces ({exc})")


def phonon_cache_load_relaxed(cache_dir, log_queue):
    """The pre-relaxed cell of the interrupted run, plus its logged summary.

    The displacements are generated from this structure, so resuming has to
    start from the very same one — a second pre-relaxation would land a hair
    away and invalidate every cached force.
    """
    path = os.path.join(cache_dir, 'pre_relaxed.extxyz')
    if not os.path.isfile(path):
        return None, {}
    try:
        relaxed = read(path, format='extxyz')
        summary = {}
        info_path = os.path.join(cache_dir, 'pre_relaxed.json')
        if os.path.isfile(info_path):
            with open(info_path) as handle:
                summary = json.load(handle)
        return relaxed, summary
    except Exception as exc:
        log_queue.put(f"  ⚠️ Cached pre-relaxed structure unusable ({exc}) — relaxing again")
        return None, {}


def phonon_cache_store_relaxed(cache_dir, relaxed, summary, log_queue):
    try:
        write(os.path.join(cache_dir, 'pre_relaxed.extxyz'), relaxed.copy(),
              format='extxyz')
        with open(os.path.join(cache_dir, 'pre_relaxed.json'), 'w') as handle:
            json.dump(summary, handle, indent=2)
    except Exception as exc:
        log_queue.put(f"  ⚠️ Could not cache the pre-relaxed structure ({exc})")
...
def calculate_phonons_pymatgen(atoms, calculator, phonon_params, log_queue, structure_name):
    try:
        ...
        cache_dir = phonon_force_cache_dir(calculator, phonon_params,
                                           structure_name, atoms, log_queue)
        if cache_dir:
            log_queue.put(f"  Force cache: {cache_dir}")
            log_queue.put("    (each displacement is stored there; restarting this "
                          "run with the same settings resumes from it)")

        pre_relax_converged  = None
        pre_relax_final_fmax = None

        cached_relaxed, cached_relax_info = (
            phonon_cache_load_relaxed(cache_dir, log_queue)
            if cache_dir and phonon_params.get('pre_relax', True) else (None, {})
        )
        if cached_relaxed is not None:
            atoms = cached_relaxed
            atoms.calc = calculator
            pre_relax_converged  = cached_relax_info.get('converged')
            pre_relax_final_fmax = cached_relax_info.get('final_fmax')
            log_queue.put("  ♻️ Reusing the pre-relaxed cell from the cache "
                          "(the cached forces belong to it)")
            if pre_relax_final_fmax is not None:
                log_queue.put(f"     F_max after that pre-relax: {pre_relax_final_fmax:.4f} eV/Å")

        if cached_relaxed is None and phonon_params.get('pre_relax', True):
            ...
            try:
                ...
                if cache_dir:
                    phonon_cache_store_relaxed(
                        cache_dir, atoms,
                        {'energy': float(energy),
                         'final_fmax': float(pre_relax_final_fmax),
                         'converged': bool(pre_relax_converged)},
                        log_queue,
                    )
        ...
        _manual_kpath_selected = bool(phonon_params.get('manual_kpath')) and \
            not phonon_params.get('use_auto_kpath', True)
        if _manual_kpath_selected:
            log_queue.put("  Manual k-path: pinning primitive_matrix=None so the "
        ...
        for i, supercell in enumerate(supercells):
            ...
            slot = _phonon_cache_slot(cache_dir, i) if cache_dir else None
            if slot is not None:
                cached = phonon_cache_load_forces(slot, ase_sc)
                if cached is not None:
                    forces.append(cached)
                    n_reused += 1
                    if n_reused == 1 or (i + 1) % max(1, n_displacements // 10) == 0:
                        log_queue.put(
                            f"    Forces: {i+1}/{n_displacements} — ♻️ reused from "
                            f"{os.path.basename(slot)}"
                        )
                    continue
            ...
            try:
                ...
                forces.append(displacement_forces)
                if slot is not None:
                    phonon_cache_store_forces(slot, ase_sc, displacement_forces,
                                              calculator, log_queue)
        ...
        if n_reused:
            log_queue.put(
                ...
                f"from the cache, {n_displacements - n_reused} were computed now"
            )
...
def _attach_orbmol_charge_spin(calculator, charge, spin, model=None):
    """Wrap ORBCalculator.calculate so:
    - atoms.info['charge']/'spin'] are guaranteed on every call (optimizers/filters can swap atoms)
    - per-atom latent charges (OrbMol-v2) are captured into calculator.results['latent_charges']
      via a forward hook on the LatentChargeHead module.
    """
    try:
    ...
    _charge = int(charge)
    _spin = int(spin)
    _orig = calculator.calculate

    _slot = {}
    if model is not None:
        try:
            for _name, _sub in model.named_modules():
                if type(_sub).__name__ == "LatentChargeHead":
                    def _hook(_module, _inp, _out, _s=_slot):
                        try:
                            _s['v'] = _out.detach().cpu().numpy()
                        except Exception:
                            pass
                    _sub.register_forward_hook(_hook)
                    break
        except Exception:
            pass

    def _patched_calculate(atoms=None, properties=None, system_changes=all_changes):
        if atoms is not None:
            atoms.info["charge"] = _charge
            atoms.info["spin"] = _spin
        _slot.pop('v', None)
        ret = _orig(atoms=atoms, properties=properties, system_changes=system_changes)
        if _slot.get('v') is not None:
            calculator.results['latent_charges'] = _slot['v']
        return ret
...
MODEL_FAMILIES = {
    ...
    "ORB": {
        ...
        "OrbMol-v1 Conservative (Molecular, OMol25)": "orbmol_v1_conservative",
        "OrbMol-v1 Direct (Molecular, OMol25)": "orbmol_v1_direct",
...
def is_orbmol_model(selected_model_name, model_size=None):
    """OrbMol family models (orbmol_v1_*, orbmol_v2). Require atoms.info['charge'] and atoms.info['spin']."""
    if selected_model_name and "ORBMOL" in selected_model_name.upper():
        return True
    if isinstance(model_size, str) and model_size.lower().startswith("orbmol"):
        return True
    return False
    ...
    try:
        ...
        if is_qe:
            ...
            if is_orbmol:
                ...
                _orbmol_charge = int(_ps.get('charge', 0))
                _orbmol_spin = int(_ps.get('spin', 1))
                log_queue.put(f"  OrbMol total charge:     {_orbmol_charge}")
                log_queue.put(f"  OrbMol spin (2S+1):      {_orbmol_spin}")
            ...
            try:
                ...
                if is_orbmol and model_size == "orbmol_v2":
                    try:
                        orbff.enable_stress()
                        log_queue.put("  ⚡ OrbMol-v2 stress prediction enabled")
                    except Exception as _se:
                        log_queue.put(f"  ⚠️ OrbMol-v2 enable_stress() failed: {_se}")
                ...
                if is_orbmol:
                    _attach_orbmol_charge_spin(calculator, _orbmol_charge, _orbmol_spin, model=orbff)
                    log_queue.put(
                        f"  ⚡ OrbMol charge/spin auto-injection enabled "
                        f"(charge={_orbmol_charge}, spin={_orbmol_spin})"
                    )
            ...
            for i, (name, structure) in enumerate(structure_data.items()):
                ...
                try:
                    ...
                    if is_orbmol:
                        ...
                        atoms.info["spin"] = int(_ps.get("spin", 1))
                        log_queue.put(
                            f"  OrbMol metadata set — charge={atoms.info['charge']}, "
                            f"spin={atoms.info['spin']}"
                        )
                        ...
                        try:
                            ...
                            if is_tetragonal or (opt_type == "Cell only (fixed atoms)" and tetragonal_callback):
                                ...
                                for step in range(optimization_params['max_steps']):
                                    ...
                                    # Convergence — force (and stress for cell modes) only,
                                    # matching standard ASE optimizer behavior. Energy change is
                                    # logged for reference but is NOT required for convergence.
                                    energy_change = None
                                    ...
                                    if converged:
                                        ...
                                        if energy_change is not None:
                                            log_queue.put(
                                                f"     Energy change: {energy_change:.2e} eV (informational)")
                                        break
                    ...
                    orbmol_results = None
                    if is_orbmol and energy is not None:
                        orbmol_results = extract_orbmol_results(
                            atoms, calculator, log_queue, name
                        )
...
with st.sidebar:
    ...
    if is_polar_model(selected_model) and not use_qe:
    ...
    elif is_orbmol_model(selected_model, model_size):
        st.markdown("---")
        _is_v2 = (model_size == "orbmol_v2")
        st.subheader("⚡ OrbMol Settings" + (" (v2 — Learnable Electrostatics)" if _is_v2 else ""))

        if not ORB_AVAILABLE:
            st.error(
                "❌ `orb-models` not found — install: `pip install orb-models`"
            )
        ...
        if _is_v2:
            st.caption(
                "OrbMol-v2 — trained on OMol25 + OPoly26 (ωB97M-V/def2-TZVPD)."
            )
        else:
            st.caption(
                "OrbMol v1 (conservative/direct) — trained on OMol25 (ωB97M-V/def2-TZVPD). "
                "Note: training data does not contain periodic systems."
            )
        st.caption(
            "📄 [Orb models docs](https://github.com/orbital-materials/orb-models)"
        )
...
with tab1:
    ...
    if True:
            ...
            with tab_gen:
                ...
                if st.session_state.generated_energy_grid_script:
                    ...
                    st.info(
                        ...
                        "6. Switch to the **📊 Output analysis** tab and upload an "
                        "`energy_grid.npz` to render 3-D volume / 2-D slice views."
                    )
        ...
        if calc_type == "Geometry Optimization":
            ...
            if use_qe:
                st.write("### ♻️ Resume interrupted runs")
                phonon_params["resume_from_cache"] = st.checkbox(
                    "Cache the forces of every displaced supercell",
                    value=_pg('resume_from_cache', True),
                    help="Each displaced supercell is stored in its own folder under "
                         "<QE working directory>/phonon_cache/, together with its "
                         "espresso.pwi/espresso.pwo. If the run is interrupted, "
                         "starting it again with the same settings skips every "
                         "displacement that already finished and continues with the "
                         "rest. Changing the cutoffs, k-points, pseudopotentials, "
                         "supercell, displacement distance or pre-relax settings "
                         "starts a fresh cache, so stale forces are never reused.")
                if phonon_params["resume_from_cache"]:
                    _qe_work_dir = os.path.abspath(os.path.expanduser(
                        qe.get_active_qe_settings().get('work_dir') or 'qe_calc'))
                    st.caption(f"Cache location: `{os.path.join(_qe_work_dir, 'phonon_cache')}`")
            ...
            _PHONON_PERSIST_KEYS = [
                ...
                'calc_gamma_irreps', 'resume_from_cache',
            ]
                ...
                with st.expander("📚 Understanding ORB-v3 Confidence Predictions", expanded=False):
                    st.markdown("""
                            ...
                            **Example:** If the predicted force on an atom is 0.15 eV/Å but the true force is 0.20 eV/Å, 
                            ...
                            """)
            # ── OrbMol-v2 per-atom latent charges ─────────────────────────────
            orbmol_charge_results = [
                r for r in st.session_state.results
                if r.get('orbmol_results') and r['orbmol_results'].get('success')
            ]

            if orbmol_charge_results:
                st.markdown("---")
                st.subheader("⚡ OrbMol Per-Atom Latent Charges")

                if len(orbmol_charge_results) == 1:
                    _sel_orbmol = orbmol_charge_results[0]
                else:
                    _orbmol_names = [r['name'] for r in orbmol_charge_results]
                    _oname = st.selectbox(
                        "Select structure for charge analysis:",
                        _orbmol_names, key="orbmol_charges_sel")
                    _sel_orbmol = next(r for r in orbmol_charge_results if r['name'] == _oname)

                om_data = _sel_orbmol['orbmol_results']
                om_charges = np.array(om_data['charges'], dtype=float)

                _struct_for_symbols = _sel_orbmol.get('structure')
                try:
                    _symbols = [str(s.specie.symbol) for s in _struct_for_symbols.sites]
                except Exception:
                    _symbols = [f"atom{i}" for i in range(len(om_charges))]

                col_om1, col_om2, col_om3 = st.columns(3)
                with col_om1:
                    st.metric("Min charge (e)", f"{om_data['charge_min']:.4f}")
                with col_om2:
                    st.metric("Max charge (e)", f"{om_data['charge_max']:.4f}")
                with col_om3:
                    st.metric("Sum charge (e)", f"{om_data['charge_sum']:.4f}")

                fig_om = go.Figure()
                _xlabels = [f"{i}: {sym}" for i, sym in enumerate(_symbols)]
                _colors = ['#d62728' if c > 0 else '#1f77b4' for c in om_charges]
                fig_om.add_trace(go.Bar(
                    x=list(range(len(om_charges))),
                    y=om_charges,
                    marker_color=_colors,
                    marker_line_color='black',
                    marker_line_width=1.2,
                    text=[s for s in _symbols],
                    textposition='outside',
                    hovertemplate='<b>Atom %{x} (%{text})</b><br>Charge: %{y:.4f} e<extra></extra>',
                ))
                fig_om.add_hline(y=0.0, line_dash="dash", line_color="grey", line_width=1)
                fig_om.update_layout(
                    title=dict(text="Per-Atom Latent Charge (e)", font=dict(size=22)),
                    xaxis_title="Atom Index",
                    yaxis_title="Charge (e)",
                    height=520,
                    font=dict(size=16),
                    hoverlabel=dict(bgcolor="white", font_size=16, bordercolor="black"),
                )
                st.plotly_chart(fig_om, width='stretch')

                _df_om = pd.DataFrame({
                    "Atom Index": list(range(len(om_charges))),
                    "Element": _symbols,
                    "Latent Charge (e)": [f"{c:.4f}" for c in om_charges],
                })
                with st.expander("Per-atom table", expanded=False):
                    st.dataframe(_df_om, width='stretch', hide_index=True)

                st.caption(
                    "ℹ️ OrbMol-v2 predicts per-atom *latent* charges from a `LatentChargeHead` "
                    "(constrained to sum to the total system charge). These are **emergent** from "
                    "energy/force optimisation — the model never saw per-atom charge labels during "
                    "training, so treat the values qualitatively."
                )
