...
def _phonon(p):
    ...
    if use_auto_kpath:
        ...
        lines += [
            ...
            f"npts = {npts}",
            "bands = []",
            "# One band per *branch* of the path: pymatgen splits the path at",
            "# discontinuities (the '|' in Γ-X-M-Γ-Z-R-A-Z|X-R|M-A), and each",
            "# branch must stay a separate path so is_band_connection=True does",
            "# not connect bands across the jump.",
            "for seg in path_segs:",
            "    branch = []",
            "    for i in range(len(seg) - 1):",
            "        start = np.array(kpoints_dict[seg[i]])",
            "        end   = np.array(kpoints_dict[seg[i+1]])",
            "        pts   = [start + t/(npts-1) * (end - start) for t in range(npts)]",
            "        branch += pts",
            "    bands.append(branch)",
        ]
    else:
