...
def _build_cube_from_loaded_npz(data, smoothing_sigma=0.0, zoom_factor=1.0,
    ...
    lines.append(f"   1   0.000000   0.000000   0.000000")
    ...
    # Probe atom anchored at the cell origin.
    lines.append(
        f"  {z_atom:3d}   0.000000   0.000000   0.000000   0.000000"
    )
...
def _build_xsf_from_loaded_npz(data, smoothing_sigma=0.0, zoom_factor=1.0,
    ...
    *smoothed / interpolated* field that's currently on screen. The probe
    ...
    lines.append(f"{z_atom:>3d}   0.0000000   0.0000000   0.0000000")
...
def _write_xsf(path, atoms, energies_3d, blocked, origin, cell):
...
{calculator_setup_code}
    ...
    for fname in structure_files:
        ...
        na = max(2, int(np.ceil(a_len / GRID_SPACING_A)))
        nb = max(2, int(np.ceil(b_len / GRID_SPACING_A)))
        nc = max(2, int(np.ceil(c_len / GRID_SPACING_A)))
        n_total = na * nb * nc
        print(
        ...
        print(
            ...
            f"(actual spacings: {{a_len/na:.3f}}, {{b_len/nb:.3f}}, "
            f"{{c_len/nc:.3f}} Å)"
        )
        ...
        if SAVE_MIN_STRUCT and (~blocked).any():
            try:
                ...
                _frac_min = np.array([_ci / na, _cj / nb, _ck / nc])
